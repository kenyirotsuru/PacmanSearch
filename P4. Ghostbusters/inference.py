...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for position in self.allPositions:
            pastBelief = self.beliefs[position]
            self.beliefs[position] = self.getObservationProb(observation, gameState.getPacmanPosition(), position, self.getJailPosition())*(pastBelief)
        self.beliefs.normalize()
...
class ParticleFilter(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for particle in range(0, len(self.particles)-1):
            weight[particle] = self.getObservationProb(observation, gameState.getPacmanPosition(), self.particles[particle], self.getJailPosition())

        if weight.total() != 0:
            weight.normalize()
            for i in range(0, len(self.particles)-1):
                self.particles[i] = self.particles[weight.sample()]
        else:
            self.initializeUniformly(gameState)
    ...
    def elapseTime(self, gameState):
        ...
        for particle in range(0, len(self.particles)-1):
            newPosDist = self.getPositionDistribution(gameState, self.particles[particle])
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for particle in self.particles:
            ...
            for i in range(self.numGhosts):
                temp *= self.getObservationProb(observation[i], pacmanPosition, particle[i], self.getJailPosition(i))
            weight[particle] += temp

        if weight.total() != 0:
            weight.normalize()
            for particle in range(self.numParticles):
                self.particles[particle] = weight.sample()
        else:
            self.initializeUniformly(gameState)
